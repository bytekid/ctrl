...
module Xml : sig
  module MakeParser(P : CHAR_PARSER) : sig
    ...
    val parse : P.input -> (Error.t,Util.Xml.t)Util.either
  end
  ...
  module type TRANSFORMER = sig
    ...
    val any : 'a t -> (string * Util.Xml.attrs * string option * 'a)t
    ...
    it's value. *)
    val leaf : (string * Util.Xml.attrs * string option)t
    ...
    val run : 'a t -> state -> Util.Xml.node -> (string,'a)Util.either
