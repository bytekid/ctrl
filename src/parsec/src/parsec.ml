...
module Xml = struct
  ...
  module MakeLexer(P : CHAR_PARSER) = struct
    ...
    module Token = struct
      type token = Close of string
                 | Comment
                 | Empty of (string * Xml.attrs)
                 | Meta of (string * Xml.attrs)
                 | Open of (string * Xml.attrs)
                 | Text of string
      ;;
    ...
    let text =
      many1 (noneof "<>") >>= fun cs ->
      get_position        >>= fun e ->
      return (e, Token.Text (String.of_char_list (List.drop_while Char.is_space cs)))
    ;;
  ...
  module MakeParser(P : CHAR_PARSER) = struct
   ...
   let text = token (function Tok.Text "" -> None
                            | Tok.Text txt -> Some txt
                            | _ -> None);;
   ...
   let rec node () = (
    (tage >>= fun (id, attrs) -> return (Xml.mk_node id attrs [] None))
    <|>
    (
      tago            >>= fun (id, attrs) ->
      skip_comments   >>
      ...
      skip_comments   >>
      tagc            >>= fun id' ->
      if id = id' then return (Xml.mk_node id attrs chs txt)
  ...
  module type TRANSFORMER = sig
    ...
    val (<?>) : string -> 'a t -> 'a t
    val (<*>) : unit t
    val any : 'a t -> (string * Xml.attrs * string option * 'a)t
    val bool : bool t
    val choice : 'a t list -> 'a t
    val empty : unit t
    val fail : string -> 'a t
    val int : int t
    val leaf : (string * Xml.attrs * string option)t
    val many : 'a t -> 'a list t
    val node : 'a t -> 'a t
    ...
    val run : 'a t -> state -> Xml.node -> (string,'a)Util.either
  end

  module Transformer(State : Monad.STATE) = struct
    let to_s = Xml.to_string;;

    (*internal state; defined outside of S, to avoid having to
    use S prefix when accessing fields*)
    type s = {input : Xml.node list; state : State.t};;
    module S = struct
      type t = s;;
    end
    ...
    let return = M.return;;
    let (<$) m f = m >>= (return <.> f);;
    let fail e = M.lift (Monad.Error.fail e);;
    let catch h m s = Monad.Error.catch (flip h s) (m s);;
    let failif b e = M.lift (Monad.Error.failif b e);;
    ...
    let any args = (M.get >>= fun s -> match s.input with
      | n::ns -> (
        M.set{s with input = n.Xml.succs} >>
        (!$ args)                     >>= fun x ->
        M.get                         >>= fun s' ->
        M.set{s' with input = ns}     >>
        return (n.Xml.id, n.Xml.attrs, n.Xml.text, x)
    ...
    let tag t m = (M.get >>= fun s -> match s.input with
      | n::_ -> if n.Xml.id = t
        then m
        else fail ("unexpected node " ^ to_s n ^ "\nexpecting tag '" ^ t ^ "'")
      | _    -> err_empty ()
    );;
    ...
    let rec choice = function
      ...
      | [m;n] -> catch (fun _ -> n <? "last choice failed") m
    ...
    *)
    ...
    (* State Manipulation *)
    let get_state = M.get <$ (fun {state = s} -> s);;
