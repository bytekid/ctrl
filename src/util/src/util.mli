...
val fix : ?eq:('a -> 'a -> bool) -> ('a -> 'a) -> 'a -> 'a
(** [fix ~eq:eq f x] computes a fixed point for [f] using the equality test [eq].
By default [eq] is the standard equality [(=)]. *)
val id : 'a -> 'a
...
(** [set_debugging value] sets or unsets debugging mode.  This is used
by other modules to give extra information. *)
val query_debugging : unit -> bool
(** [query_debugging ()] returns whether we are currently in debugging
mode *)
...
module Arg : sig
 ...
 module type ARG = sig
  ...
  (* removed align because of changes to the standard library; we're
  not using it, so let's avoid conflicts with older/newer compilers *)
  val current : int ref
 ...
 val usagex: Format.formatter -> (key * spec * doc) list -> usage_msg -> unit
 (** [usagex fmt spec error_mesg] behaves similar as [usage] but without
...
module Complexity : sig
 ...
 (** {3 Miscellaneous} *)
 val get_poly_degree : t -> int
 (** [get_poly_degree c] returns the degree of the complexity classes [c] *)
 val decrease : t -> t
...
module Graph : sig
 ...
 module type SIGNATURE = sig
  ...
  returned. Otherwise only non-trivial SCCs are returned. By default,
  ...
  val find_edges : node list -> node list -> t -> (node * node) list
  (** [find_edges fs ts g] returns all edges of the graph [g] that start at some
  node in the list [fs] and end at some node in the list [ts].*)
  ...
  [1] is returned. Please do not use this function *)
  val equal : t -> t -> bool
  (** [equal g g'] checks if [g] and [g'] contain the same nodes and edges. *)
  val subgraph : t -> t -> bool
  (** [subgraph g g'] checks if all nodes and edges of [g] occur in [g']. *)
 ...
 module type LABELED = sig
  ...
  loop if [g] contains self-loops! By default [n = -1]. *)
 end
...
module Hashtbl : sig
 ...
 val add_ : 'a -> 'b -> ('a,'b) t -> ('a,'b) t
 (** [add_ k e h] adds a new binding of [k] to [e] to [h]. If there exists
 already a binding for [k] in [h], the hash table remains unchanged. Note
 that as a side effect [h] is changed too. *)
 val clear : ('a,'b) t -> unit
 val clear_ : ('a,'b) t -> ('a,'b) t
 (** [clear_ h] is equivalent to [Hashtbl.clear]. *)
 ...
 val remove_ : 'a -> ('a,'b) t -> ('a,'b) t
 (** [remove_ k h] is equivalent to [Hashtbl.remove]. Note that as a side
 effect [h] is changed too. *)
 val replace : ('a,'b) t -> 'a -> 'b -> unit
 val replace_ : 'a -> 'b -> ('a,'b) t -> ('a,'b) t
 (** [replace_ k e h] is equivalent to [Hashtbl.replace]. Note that as a
 ...
 val find : ('a,'b) t -> 'a -> 'b
 val find_ : 'a -> ('a,'b) t -> 'b
 (** [find_ k h] is equivalent to [Hashtbl.find].
 @raise Not_found if [h] does not contain a binding for [k]. *)
 val find_all : ('a,'b) t -> 'a -> 'b list
 val find_all_ : 'a -> ('a,'b) t -> 'b list
 (** [find_all_ k h] is equivalent to [Hashtbl.find_all]. *)
 (*
 val search : ('a -> 'b -> bool) -> ('a,'b) t -> 'a * 'b
 ...
 *)
 ...
 val foldi : (int -> 'a -> 'b -> 'c -> 'c) -> ('a,'b) t -> 'c -> 'c
 ...
 val iteri : (int -> 'a -> 'b -> unit) -> ('a,'b) t -> unit
 ...
 val mem_ : 'a -> ('a,'b) t -> bool
 (** [mem_ k h] is equivalent to [Hashtbl.mem]. *)
 ...
 val domain : ?compare_d:('a -> 'a -> int) -> ('a,'b) t -> 'a list
 (** [domain h] returns the domain of [h]. *)
 val range : ?compare_r:('b -> 'b -> int) -> ('a,'b) t -> 'b list
 (** [range h] returns the range of [h]. *)
 val length : ('a,'b) t -> int
 val size : ('a,'b) t -> int
 (** [size h] is equivalent to [Hashtbl.length]. *)
 val hash : 'a -> int
 ...
 val compare :
  ?compare_d:('a -> 'a -> int) -> ?compare_r:('b -> 'b -> int) ->
  ('a, 'b) Hashtbl.t -> ('a, 'b) Hashtbl.t -> int
 ...
 (** [fprintf fk fd fmt h] writes pretty printed output of [h].
 [fk] is used to format the keys, and [fd] is used to format the values. *)
 val to_string :
  ?delim:(unit, Format.formatter, unit) format ->
  (Format.formatter -> 'a -> unit) ->
  (Format.formatter -> 'b -> unit) -> ('a, 'b) t -> string
 (** [to_string fk fd h] transforms [h] into a formatted string.
 [fk] is used to format the keys, and [fd] is used to format the values. *)
...
(** 64-bit Integers
@author Simon Legner
@since  Thu Apr 26 21:52:17 CEST 2012 *)

(** This module collects some frequently used operations on 64-bit integers. *)
module Int64 : sig
 (*** TYPES *******************************************************************)
 type t = int64
 ...
 (** [succ n] computes [n+4]. *)
 val add : t -> t -> t
 (** [add n m] computes [n+m]. *)
 val pred : t -> t
 (** [pred n] computes [n-1]. *)
 val neg : t -> t
 (** [neg n] computes [-n]. *)
 val sub : t -> t -> t
 (** [sub n m] computes [n-m]. *)
 val mul : t -> t -> t
 (** [mul n m] computes [n*m]. *)
 val div : t -> t -> t
 (** [div n m] computes [n/m]. *)
 val rem : t -> t -> t
 (** [rem n m] computes [n%m]. *)
 val pow : t -> t -> t
 (** [pow n m] computes [n^m]. *)
 val abs : t -> t
 (** [pow n] computes the absolute value of [n]. *)

 (** {3 Bit Operations} *)

 val bit_max_int : int -> t
 (** [bit_max_int n] returns the largest number representable with [n] bits. *)
...
module List : sig
 ...
 compare function [c]. By default, the compare function [compare] is used. *)
 ...
 using the compare function [c]. By default, the compare function [compare]
 is used. *)
 val replace : ?c:('a -> 'a -> int) -> 'a -> 'a -> 'a list -> 'a list
 (** [replace ~c:c x y xs] replaces all occurrences of [x] in [xs] by
 [y] using the compare function [c]. By default, the compare function
 [compare] is used. *)
 ...
 val partition_option : ('a -> 'b option) -> 'a list -> 'b list * 'a list
 (** [map_partition f xs] is equivalent to {!val: Util.List.map_option} just
 that elements for which [f] returns None are included in the second returned
 list. *)
 ...
 compare function [c] to check equivalence. By default, the compare function
 [compare] is used. *)
 ...
 only equal elements. Equality is checked via the compare function [c]. By
 default [c = compare]. If [xs] is the empty list, [\[\]] is returned. *)
 val group_fully : ?c:('a -> 'a -> int) -> 'a list -> 'a list list
 (** [group_fully ~c:c xs] returns a list of lists such that the
 concatenation of the result contains the same elements as [xs], but
 all equivalent elements equal under [c] are joined in a list.  By
 default [c = compare].  If [xs] is the empty list, [[]] is returned. *)
 ...
 list of pairs [xs]. Equality is tested using the compare function [c]. By
 default [c = compare].
 ...
 [x]. Equality is tested using the compare function [c]. By default [c] is the
 ...
 that occur in [ys]. Equality is tested using the compare function [c]. By
 default [c = compare]. *)
 ...
 [c] is used. By default [c = compare]. *)
 ...
 (** [is_supset ~c:c xs ys] is equivalent to [is_subset ~c:c ys xs]. By
 default [c = compare]. *)
 ...
 The function [c] is used to compare elements. By default the compare function
 [compare] is used. Note that the order of the elements is preserved, i.e., the
 first occurrence of an element is kept. *)
val sort_unique : ?c:('a -> 'a -> int) -> 'a list -> 'a list
 (** [unique ~c:c xs] returns the list [xs] without any duplicated elements,
 sorted by the given compare function [c] (which is also used to identify
 duplicates).  By default the compare function [compare] is used. *)
 ...
 Elements are copied via the function [c]. By default [c = id]. *)
 ...
 (** [join f d xs] transforms elements from [xs] into a non-formatted string
 separated by [d]. *)
 val joini : (int -> 'a -> string) -> string -> 'a list -> string
 (** [joini f d xs] transforms elements from [xs] into a non-formatted string
 ...
 separated by [d]. Note that [f] receives as first argument the index of the
 considered element, where the first element has index [0]. *)
 val implode : ('a -> string) -> string -> 'a list -> string
 (** [implode f d xs] is like to_string f d xs, but does not use the
 formatter *)
end
...
module Monad : sig
 ...
 module type SIGNATURE = sig
  ...
  {!val: Util.Monad.SIGNATURE.liftM} function. I.e.,
  [liftM_i f m_1 ... m_i = ap (... (ap (return f) m_1) ...) m_i]
  where [f] is an [i]-ary function. *)
  val join : 'a t t -> 'a t
  (** [join m] removes one level of monadic structure by projecting its
  argument into the outer level. *)
  val liftM : ('a -> 'b) -> 'a t -> 'b t
  (** [liftM f m] promotes the function [f] to the monad [m]. *)
  val liftM2 : ('a -> 'b -> 'c) -> 'a t -> 'b t -> 'c t
  (** [liftM2 f m n] promotes the function [f] to the monad [m] and [n]. *)
  val liftM3 : ('a -> 'b -> 'c -> 'd) -> 'a t -> 'b t -> 'c t -> 'd t
  (** [liftM3 f m n o] promotes the function [f] to the monads [m], [n], and
  ...
  its result is encapsulated in a monad. *)
  val iteri : (int -> 'a -> 'b t) -> 'a list -> unit t
 ...
 module Transformer : sig
  ...
  module type COMBINED_MONAD = sig
   ...
   val lift : 'a m -> 'a t
   (** [lift m] lifts monad [m] into monad [t]. *)
  end
...
module String : sig
 ...
 In addition, after each newline [n] white spaces are added. By default
 [n = 0]. *)
 ...
 are added. By default [n = 0]. Furthermore, if some text has more than [i]
 ...
 white spaces are added. By default [n = 0]. Furthermore, if some text has
...
(** Bare-Bones Xml Type
@author Christian Sternagel 
@since  Wed Apr 27 15:38:39 CEST 2011 *)
...
module Xml : sig
  ...
  val  mk_node : string -> attrs -> node list -> string option -> node
  val fprintf : Format.formatter -> node -> unit
