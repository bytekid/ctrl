...
let partition_option f lst =
  let rec act (somes, nones) elem = match f elem with
    | None -> (somes, elem :: nones)
    | Some x -> (x :: somes, nones)
  in
  Pair.apply rev rev (foldl act ([], []) lst)
;;
...
let find_option p xs = try Some (find p xs) with Not_found -> None;;
...
let joini f d lst =
  let rec joini i = function
    | [] -> ""
    | [x] -> f i x
    | x::xs -> (f i x) ^ d ^ (joini (i+1) xs)
  in
  joini 0 lst
;;
...
let rec implode f d = function
  | [] -> ""
  | [single] -> f single
  | head :: tail -> (f head) ^ d ^ (implode f d tail)
;;
