...
module Labeled (L : LABEL) (N : NODE) = struct
 ...
 type state = {
  mutable stacked : (N.t, unit) H.t; (* stacked nodes *)
  mutable graph   : t; (* input-graph *)
  mutable gidx    : int; (* global index *)
  mutable llnn    : (N.t, int) H.t; (* low-link numbers of nodes *)
  mutable nidx    : (N.t, int) H.t; (* node index *)
 ...
 let find_edges fs ts g =
   List.fold_left (fun es f ->
     if M.mem f g then (
       let ns = M.find f g in
       List.fold_left (fun es t ->
         if R.exists (fun _ n -> N.equal t n) ns
           then (f, t) :: es
           else es
       ) es ts
     ) else es
 ...
 let equal g0 g1 = 
  (* M.equal g0 g1;; *) 
  Listx.equal (nodes g0) (nodes g1) &&
  Listx.equal (edges g0) (edges g1)
 ;;
 let subgraph g0 g1 = 
  Listx.is_subset (nodes g0) (nodes g1) &&
  Listx.is_subset (edges g0) (edges g1)
 ;;
