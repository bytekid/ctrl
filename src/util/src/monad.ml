...
module Make (M : MINIMAL_MONAD) = struct
 ...
 let join m = m >>= id;;
 let liftM f = ap (return f);;
 let liftM2 f m = ap (liftM f m);;
 let liftM3 f m n = ap (liftM2 f m n);;
 ...
 let iteri f = let rec go i = function
   | [] -> return ()
   | x :: xs -> f i x >>= fun _ -> go (i+1) xs
   in
   go 0;;
 let iter f xs = iteri (const f) xs;;
 ...
 let exists p = liftM not <.> for_all (liftM not <.> p);;
 ...
 (* Pair Functions *)
 let cross (f,g) (x,y) = f x >>= (flip liftM (g y) <.> Pair.make);;
...
module Transformer = struct
 ...
 module State (S : STATE) (M : MINIMAL_MONAD) = struct
  ...
  (*** FUNCTIONS **************************************************************)
  let lift m = (fun s -> M.(>>=) m (flip return s));;
